...
- [x] Quick-filter coverage guard filters weak first-word-only overlaps early (minimum shared-token/coverage gate) while keeping deterministic full-candidate fallback with verbose reason logging (`src/match/engine.py`, `tests/test_matching.py`, `docs/CLI.md`)
...
- [x] Added conservative qwen3 runtime knobs (`--qwen3-batch-size`, `--qwen3-chunk-length-s`) as optional backend-specific controls with deterministic validation and signature-gated forwarding (no behavior change when omitted) (`cli.py`, `src/asr/config.py`, `src/asr/asr_worker.py`, `src/asr/qwen3_asr_backend.py`, `tests/test_cli.py`, `tests/test_qwen3_asr_backend.py`, `docs/CLI.md`)
...
- [x] Alignment preflight-only CLI mode (`--alignment-preflight-only`) for alignment-backend availability/model resolution/device probe sanity checks with deterministic one-line JSON output and no media/script/output requirements (`cli.py`, `tests/test_cli.py`, `docs/CLI.md`)
- [x] Runnable explicit alignment CLI mode (`--alignment-backend`) using existing `src/align/` backend contract + validation with deterministic non-empty timestamped spans bridged into matching/export flow (`cli.py`, `tests/test_cli.py`, `docs/CLI.md`, `docs/Integration.md`)
- [x] Optional explicit two-stage qwen3 orchestration mode (`--two-stage-qwen3`) runs stage-1 `qwen3-asr` transcript flow and stage-2 `qwen3-forced-aligner` on deterministic `reference_spans[]`, then feeds validated aligned timestamps into matching/export without fabricating timestamps; includes mocked success/failure/verbose-stage CLI tests (`cli.py`, `tests/test_cli.py`)
- [x] Deterministic script-row→`reference_spans[]` converter for alignment mode with stable file-order mapping, empty/invalid row skipping, and verbose-only skip diagnostics (`src/align/reference_spans.py`, `cli.py`, `tests/test_reference_spans.py`, `tests/fixtures/script_alignment_reference_spans.tsv`, `docs/Data-contracts.md`, `docs/CLI.md`)
...
- [x] qwen3-asr runtime kwarg contract hardening: transcribe kwargs are signature-filtered (no unconditional `return_timestamps`, no default temperature pass-through), unsupported decode knobs remain deterministic, and worker subprocess verbose/non-verbose failure visibility is regression-tested (`src/asr/qwen3_asr_backend.py`, `tests/test_qwen3_asr_backend.py`, `tests/test_cli.py`, `docs/Integration-issues.md`, `docs/Data-contracts.md`)
- [x] qwen3-asr transcribe signature-inspection typing fix: kwargs filter now looks up `transcribe` via `getattr`, guards `inspect.signature` behind a callable check, and preserves deterministic fallback when signature extraction is unavailable (`src/asr/qwen3_asr_backend.py`, `tests/test_qwen3_asr_backend.py`)
...
- [x] Matching quality tuning guidance documented with deterministic long-media/strictness presets, explicit `--chunk 0` stability caveat, fuzzy-match pitfalls cross-link, and CLI defaults regression coverage for discoverability (`cli.py`, `docs/CLI.md`, `docs/Integration-issues.md`, `tests/test_cli.py`, `docs/STATUS.md`)
- [x] Deterministic ASR input strategy selection via `--asr-chunk-mode` (`auto|canonical|per-chunk`) with stable per-chunk ordering, absolute-offset merge, sequential segment ID rebuild, and docs/parser parity coverage (`cli.py`, `tests/test_cli.py`, `tests/test_docs_consistency.py`, `docs/CLI.md`, `docs/Integration.md`, `docs/STATUS.md`)
- [x] Focused CLI orchestration coverage for ASR chunk modes: mocked multi-chunk preprocess metadata verifies per-chunk dispatch count and chunk WAV path routing, absolute timestamp merge (`local + offset`), deterministic segment ID resequencing/order, canonical single-dispatch behavior, and deterministic empty-chunk failure path (`cli.py`, `tests/test_cli.py`, `docs/STATUS.md`)
...
## Change log (manual)
- 2026-02-13 – Addressed review feedback on ASR source-mode diagnostics by routing new verbose ASR input/chunk progress lines through a dedicated CLI logger configured to emit plain diagnostic messages to stderr (instead of stdout), preserving deterministic message content while keeping stdout focused on primary pipeline output; updated CLI tests to assert verbose ASR mode logs from stderr. (`cli.py`, `tests/test_cli.py`, `docs/STATUS.md`).
- 2026-02-13 – Made verbose ASR source mode explicit for debugging long-audio quality issues: CLI now logs either `ASR input: canonical wav` or `ASR input: N chunks (chunk_seconds=...)` before ASR dispatch, and `per-chunk` mode now prints deterministic per-chunk progress lines with chunk order/index, time range + offset, chunk file basename, and returned segment count; added CLI regression coverage for canonical + per-chunk verbose log content (`cli.py`, `tests/test_cli.py`, `docs/STATUS.md`).
- 2026-02-13 – Addressed ASR chunk-mode review feedback: per-chunk orchestration now preserves `chunk_index` through merged intermediate segments so existing cross-chunk continuity logic can apply deterministic boundary merges, and segment ID renumbering now preserves future segment fields by copying segment payloads while overriding only `segment_id`; updated CLI regression coverage for tuple shape/chunk-index propagation. (`cli.py`, `tests/test_cli.py`, `docs/STATUS.md`).
- 2026-02-13 – Added deterministic ASR input strategy selection flag `--asr-chunk-mode {auto,canonical,per-chunk}` in CLI orchestration: `canonical` runs ASR once on canonical WAV, `per-chunk` iterates stable `chunk_index` order and offsets merged timestamps by `absolute_offset_seconds`, `auto` preserves current canonical policy; merged outputs now rebuild sequential `seg_####` IDs independent of backend-local IDs while preserving deterministic ASR meta. Added CLI/docs consistency regression coverage and updated CLI/Integration docs (`cli.py`, `tests/test_cli.py`, `tests/test_docs_consistency.py`, `docs/CLI.md`, `docs/Integration.md`, `docs/STATUS.md`).
...
- 2026-02-13 – Refactored `_validate_backend` Optional narrowing to use explicit `status is None`/alignment guards (no CLI behavior change), and added CLI regression coverage for unknown-backend (missing status row) plus alignment-backend ASR-mode rejection paths (`cli.py`, `tests/test_cli.py`, `docs/STATUS.md`).
- 2026-02-13 – Closed text-only timestamp contract/runtime gap in ASR→matching orchestration: CLI now detects `timestamp_guarantee="text-only"` transcript-only failures, raises deterministic actionable forced-alignment guidance (no fabricated timestamps, no silent matching continuation), and added CLI regression coverage plus Integration/Data-contract updates (`cli.py`, `tests/test_cli.py`, `docs/Integration.md`, `docs/Data-contracts.md`, `docs/STATUS.md`).
...
- 2026-02-13 – Tightened matching quick-filter acceptance with an additional minimum-coverage gate (minimum shared tokens or max-set coverage), applied the gate before candidate scoring, added deterministic verbose fallback diagnostics when all candidates are rejected, added matching tests for first-word-only false positives and legitimate short-line matches, and documented the threshold semantics (`src/match/engine.py`, `tests/test_matching.py`, `docs/CLI.md`, `docs/STATUS.md`).
- 2026-02-13 – Refined quick-filter coverage gate implementation to compute overlap stats once (no redundant set/intersection work), and fixed explicit empty-token semantics so empty/empty comparisons remain deterministic pass-through while one-sided empty inputs are rejected; added focused unit tests for these edge cases (`src/match/engine.py`, `tests/test_matching.py`, `docs/STATUS.md`).
...
- [x] ASR→matching bridge deterministic timestamp gate for text-only backends: when `timestamp_guarantee="text-only"` runtime output lacks timestamps, CLI fails fast with forced-alignment guidance and never continues to matching; added CLI regression tests for rewritten text-only failure + non-text-only passthrough (`cli.py`, `tests/test_cli.py`, `docs/Integration.md`, `docs/Data-contracts.md`, `docs/STATUS.md`).

- 2026-02-13 – Added dedicated alignment preflight-only CLI path (`--alignment-preflight-only`) that reuses existing backend capability validation/model resolution/device probing, emits deterministic single-line JSON, skips media/script/output requirements for alignment validation, and added CLI regression coverage for success/failure/payload shape plus docs updates (`cli.py`, `tests/test_cli.py`, `docs/CLI.md`, `docs/STATUS.md`).

- 2026-02-13 – Audited qwen3 runtime knobs and added conservative optional CLI controls (`--qwen3-batch-size`, `--qwen3-chunk-length-s`) with deterministic validation, worker forwarding, and backend signature-based kwarg filtering so omitted flags keep defaults unchanged; added accepted/ignored/rejected unit coverage and documented low-memory usage profiles (`cli.py`, `src/asr/config.py`, `src/asr/asr_worker.py`, `src/asr/qwen3_asr_backend.py`, `tests/test_cli.py`, `tests/test_qwen3_asr_backend.py`, `tests/test_docs_consistency.py`, `docs/CLI.md`, `docs/STATUS.md`).
- 2026-02-13 – Added runnable explicit alignment execution path in CLI via dedicated alignment flags/backend selection, reusing existing `src/align/qwen3_forced_aligner.py` contract and `src/align/validation.py`, preserving ASR-mode alignment-backend rejection, and adding CLI integration-style coverage for non-empty timestamped chunk/span output (`cli.py`, `tests/test_cli.py`, `tests/test_docs_consistency.py`, `docs/CLI.md`, `docs/Integration.md`, `docs/STATUS.md`).
- 2026-02-13 – Resolved `_merge_short_segments` TypedDict invariance warning by rebuilding merged items as explicit `ASRSegment` payloads (required keys always present; optional `speaker` copied only when available), preserving deterministic merge behavior and IDs; added regression coverage asserting merged output contract key set and value types (`src/asr/faster_whisper_backend.py`, `tests/test_faster_whisper_backend.py`, `docs/STATUS.md`).

- 2026-02-13 – Followed alignment-mode review feedback by eliminating duplicate script-table loads in CLI alignment flow (single parse reused for reference span derivation + matching) and tightening `_run_alignment_mode` return typing to `ScriptTable`/`PreprocessResult`; added regression assertion that alignment path calls `load_script_table` exactly once (`cli.py`, `tests/test_cli.py`, `docs/STATUS.md`).
- 2026-02-13 – Added opt-in `--two-stage-qwen3` orchestration path (no auto-switching): stage-1 runs qwen3-asr transcript path, stage-2 runs qwen3-forced-aligner on deterministic transcript-derived `reference_spans[]`, validated aligned spans feed matching/export with no timestamp fabrication; added CLI regression tests for mocked successful run, deterministic aligner-missing failure, and verbose stage boundary logging (`cli.py`, `tests/test_cli.py`, `docs/Integration.md`, `docs/Data-contracts.md`, `docs/STATUS.md`).
- 2026-02-13 – Addressed two-stage review feedback by factoring shared forced-alignment execution into a reusable helper (used by direct alignment + two-stage paths), moving two-stage stage boundary logs into execution scope, and splitting deterministic timing metrics into `preprocess`, `asr_stage1`, and `asr_stage2` (with aggregate `asr`); added CLI/docs-consistency regression coverage updates for ordered verbose logs and new CLI flag documentation parity (`cli.py`, `tests/test_cli.py`, `tests/test_docs_consistency.py`, `docs/CLI.md`, `docs/STATUS.md`).

- 2026-02-13 – Added deterministic matching-quality tuning guidance in CLI docs (long-media chunk presets + stricter fuzzy-match presets), documented explicit `--chunk 0` long-file stability caveat, cross-linked fuzzy-match pitfalls to the tuning section, and added CLI parser-default regression coverage for matching/chunk flags discoverability (`docs/CLI.md`, `docs/Integration-issues.md`, `tests/test_cli.py`, `docs/STATUS.md`).
